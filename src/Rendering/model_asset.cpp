...
struct ModelAsset::Impl {
    ...
    struct Part { size_t node; cgltf_skin* skin; std::vector<Vertex> vertices; std::vector<size_t> indices; };
...
ModelAsset::ModelAsset(const std::string& path):m(std::make_unique<Impl>()) {
    ...
    struct Image {int w,h;std::vector<unsigned char> pixels;};std::map<const cgltf_image*,Image> images;
    auto colorAt=[&](cgltf_material* material,glm::vec2 uv) {
        glm::vec3 color(1); if(!material) return color;
        const auto& pbr=material->pbr_metallic_roughness;color=glm::make_vec3(pbr.base_color_factor);
        if(auto texture=pbr.base_color_texture.texture) {
            auto img=texture->image;if(!img) throw std::runtime_error("Missing model texture: "+path);
            if(!images.count(img)) {
                int w,h,channels;unsigned char* bytes=nullptr;stbi_set_flip_vertically_on_load(false);
                if(img->uri) bytes=stbi_load((std::filesystem::path(path).parent_path()/img->uri).string().c_str(),&w,&h,&channels,3);
                else if(img->buffer_view) bytes=stbi_load_from_memory(cgltf_buffer_view_data(img->buffer_view),int(img->buffer_view->size),&w,&h,&channels,3);
                if(!bytes) throw std::runtime_error("Cannot load model texture: "+path);
                images.emplace(img,Image{w,h,std::vector<unsigned char>(bytes,bytes+w*h*3)});stbi_image_free(bytes);
            }
            const auto& image=images.at(img);
            // The selected Kenney models use a solid-color palette (constant UVs per face).
            int x=std::clamp(int(uv.x*image.w),0,image.w-1),y=std::clamp(int(uv.y*image.h),0,image.h-1);
            auto pixel=&image.pixels[(y*image.w+x)*3];
            return displayColor(color)*glm::vec3(pixel[0],pixel[1],pixel[2])/255.0f;
        }
    ...
    for(size_t ni=0;ni<raw->nodes_count;++ni) if(auto mesh=raw->nodes[ni].mesh) {
        for(size_t pi=0;pi<mesh->primitives_count;++pi) {
            ...
            Impl::Part part{ni,raw->nodes[ni].skin,{},{}};
            for(size_t i=0;i<pos->count;++i) {
                ...
                v.color=colorAt(p.material,v.uv);
    ...
    auto vertices=m->vertices(hasAnimation("Idle")?"Idle":"",0,"",0,1);
...
std::vector<float> ModelAsset::vertices(const std::string& clip,float time,const std::string& previous,float previousTime,float blend) const {return m->vertices(clip,time,previous,previousTime,std::clamp(blend,0.0f,1.0f));}
